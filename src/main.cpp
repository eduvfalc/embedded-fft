...
main()
{
    ...
    std::chrono::milliseconds duration(
        std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::duration<double>(0.5)));

    // create signal container
    std::vector<Complex> signal = {};

    // create signal generator
    std::shared_ptr<SignalGenerator> generator = std::make_shared<SignalGenerator>(signal, duration, sampling_period);
